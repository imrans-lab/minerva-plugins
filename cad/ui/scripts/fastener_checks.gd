...
##
## WHAT THIS FILE HOLDS. The check itself: the reservation and run, the
## per-screw measurements, the ray fans, where the solid's bores come from,
## the pairing and the report. The seat ring — where the head bears and how
## much of the ring lands — and the fit table that says what a bore is FOR
## live in fastener_seat.gd, which this script extends, so one object still
