...
**Sibling:** [`PLUGIN_DEVELOPER_GUIDE.md`](./PLUGIN_DEVELOPER_GUIDE.md) §18 — where the platform's plugin contract hands off to this document.

> **Mirrored document.** The upstream source of truth is Minerva's
> `Docs/design/Annotation-substrate-plugin-adoption.md`; this copy is mirrored into the
> plugins repo so authors without a Minerva checkout can read it. Mirrored from Minerva
> `adcb58e3` (2026-08-17). If you change the substrate, update the Minerva copy first
> and re-mirror. The substrate's design contract (`Annotation-substrate-design.md`)
> lives only in Minerva and is not required to write a plugin.
...
A reference implementation of all three lives in this repo at `cad/ui/` — `CadAnnotationHost.gd`, `kinds/cad_edge_number_kind.gd`, and `tools/cad_edge_number_tool.gd`. Patterns from those files are reproduced below; you do not need access to them to write your own CAD-style plugin.
...
> This appendix is for Minerva maintainers. Plugin authors do not need to read these files — every signature and pattern they expose is already inlined in the sections above. Paths beginning `src/` are **Minerva-repo** paths; paths without a prefix are relative to **this** (plugins) repo.
...
- [`cad/ui/CadAnnotationHost.gd`](../cad/ui/CadAnnotationHost.gd) — second plugin host (off-tree).
- [`cad/ui/kinds/cad_edge_number_kind.gd`](../cad/ui/kinds/cad_edge_number_kind.gd) — custom kind with multi-pane render.
- [`cad/ui/tools/cad_edge_number_tool.gd`](../cad/ui/tools/cad_edge_number_tool.gd) — custom authoring tool with click-to-place.

Sibling design docs (Minerva repo, maintainers only):

- `Docs/design/Annotation-substrate-design.md` — substrate architecture and contract.
- `Docs/PCB-annotation-migration.md` — PCB editor migration plan.
...
- **Full CAD migration.** Semantic hit-testing, BVH-backed edge queries, real camera transforms, mesh-face anchors, and the rest of the multi-pane work are deferred. The CAD scaffold at `cad/ui/` is enough to validate the contract; it is not the finished editor.
