...
(function (global) {
  ...
  function modelSelect(id, models, chosen, emptyLabel) {
    ...
    var selectedKey = keyOf(chosen), found = false, seen = {};
    ...
    R.list(models).forEach(function (model) {
      ...
      var duplicate = !!seen[key]; seen[key] = true;
      var matches = !duplicate && !found && (key === selectedKey || (!selectedKey.startsWith('spec:') && R.text(model.model_name).toLowerCase() === selectedKey.toLowerCase()));
      if (matches) { found = true; }
      options.push(el('option', { value: key, disabled: duplicate ? true : null, selected: matches ? true : null,
        text: (model.display || model.model_name) + ' (' + model.provider_display + ')' + (duplicate ? ' — unavailable: host identity duplicates an earlier option' : '') }));
    });
    if (selectedKey && !found) { options.push(el('option', {value: selectedKey, selected: true, text: 'Unavailable: ' + (chosen && typeof chosen === 'object' ? (chosen.model_name || chosen.action_name || chosen.kind || 'saved model') : selectedKey)})); }
