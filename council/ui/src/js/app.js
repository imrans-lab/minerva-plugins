...
(function (global) {
  ...
  function handoff() {
    ...
    bridge.send('wrapper.chat_handoff', {
      ...
      if (!reply || !reply.ok) { return; }
      // A transcript longer than the host's message cap travels as consecutive
      // messages, so the count is said rather than left for the chat to explain.
      var parts = reply.payload.parts || 1;
      say('Sent ' + reply.payload.characters + ' characters to chat ' + reply.payload.chat_id
        + (parts > 1 ? ', in ' + parts + ' messages.' : '.'));
    });
  }
