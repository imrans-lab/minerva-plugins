// Package tools — minerva_pcb_fetch_libraries / minerva_pcb_library_status:
// the in-process (no Python worker) MCP tools fronting the Go-side
// library-data fetcher (pcb/internal/libraries/). See docs/libraries.md for
// the full contract.
package tools
...
var FetchLibraries = ToolSpec{
	...
	Description: "Fetch the curated KiCAD symbol/footprint library subset (pcb/libraries.lock.json) " +
		"that minerva_pcb_check_libraries/minerva_pcb_check_bom read. Downloads each locked entry, verifies it by " +
		...
		"real footprint/symbol checks — see minerva_pcb_library_status to check what's already fetched.",
...
var LibraryStatus = ToolSpec{
	...
	Description: "Report whether the KiCAD library-data subset (pcb/libraries.lock.json) has been " +
		...
		"minerva_pcb_check_libraries/minerva_pcb_check_bom will report missing_data:true — run minerva_pcb_fetch_libraries " +
