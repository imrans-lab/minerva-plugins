...
**Where a designator lands.** The board decides first: a component carrying a
`refdes_placement` block (see `board-yaml.md`) is printed exactly there, because
somebody chose it. Failing that, a footprint that authors its own `fp_text
...
panel, the geometric DRC's silk projection, the KiCad export and this emitter all
read that one rule — resolved once at compile onto `ResolvedComponent.refdes` —
so the editor draws the designator where the fab prints it.
