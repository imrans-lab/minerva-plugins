...
func initRegistry() {
	...
	// pcb.assembly_placements — the resolved logical->physical placement map
	// the panel's component tools report. See worker_tools.go.
	registry.Register(tools.AssemblyPlacementsChannel, tools.HandleAssemblyPlacementsChannel)
...
var workerBackedTools = map[string]bool{
	...
	// pcb.assembly_placements dispatches to the worker's "assembly_placements"
	// method — same rationale.
	"pcb.assembly_placements": true,
