...
This is the same wire framing the CAD plugin's worker uses (cad/worker/
mcad_worker/framing.py) — the shape is copied verbatim so both workers speak
the identical framing the shared Go bridge (shared/bridge/framing.go) expects.
It is deliberately NOT shared Python code: each worker owns its own stdio
plumbing; only the Go bridge module is shared.

Frame format (bridge design §3):
    Content-Length: NNNN\r\n
