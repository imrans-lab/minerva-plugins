...
## Real ids of the two OPEN source hints seeded in _setup() — captured so the
## canned router replies below can stamp the worker's own per-route `hint_ids`
## (docket 019f9c3a136c: a proposal's ONLY proof of proposal-hood is a
## non-empty kind_payload.proposal_for, taken verbatim from these; a canned
## reply that omits them makes every proposal it produces unattributed).
var _gnd_hint_id := ""
var _sig_hint_id := ""
...
func _setup() -> bool:
	...
	_gnd_hint_id = _seed_hint("GND", 0.4, ["U1.1"], ["R1.1"])
	_sig_hint_id = _seed_hint("SIG", 0.25, [], [])
	return true
