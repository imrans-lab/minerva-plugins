...
## Assert `required` is a SUBSET of result's keys — for shapes that may gain
## fields additively (docket 019fa57977fa: import_trace_geometry gained
## trace_ids/via_ids in 9676708 without breaking anyone reading the original
## fields). Unlike check_keys, an EXTRA key never fails this; a MISSING
## required key still does — verified by construction below (missing is
## computed by presence, not by exact-set comparison, so it can never pass
## vacuously regardless of what extra keys the result carries).
func check_keys_subset(desc: String, result: Dictionary, required: Array) -> void:
	var missing: Array = []
	for k in required:
		if not result.has(k):
			missing.append(k)
	check("%s — required keys %s all present (missing %s, got %s)" % [desc, str(required), str(missing), str(result.keys())],
		missing.is_empty())
...
func _run_wave1_dispatch_and_shape_checks() -> void:
	...
	# The interpreted destination is the spatial index's choice, not the
...
func _run_wave2_dispatch_and_shape_checks() -> void:
	...
	check_keys_subset("import_trace_geometry shape", ti, ["success", "trace_count", "via_count"])
