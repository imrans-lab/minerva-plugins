extends RefCounted
## PER-USER TOGGLE FIXTURE for the gd suites that exercise the Options snaps
## and the hover-card toggle.
##
...
## changed afterwards.
##
## `reset()` returns the toggle keys to "never chosen" (so every read sees
...
## The per-user toggles (three snaps + the hover card), and nothing else: a
## fixture that reset the whole store would also wipe `trace_width_mm`, which
## other suites pin.
const SNAP_KEYS: Array[String] = [
	_PcbPrefs.KEY_SNAP_GRID, _PcbPrefs.KEY_SNAP_LAND, _PcbPrefs.KEY_SNAP_ANGLE,
	_PcbPrefs.KEY_HOVER_CARD]


## Clear the toggle keys and return what they held, for `restore()`.
...
static func restore(saved: Dictionary) -> void:
	if saved.is_empty():
		...
		# Touching nothing is the honest answer; clearing keys nobody
