...
func _run_mirror_convention() -> void:
	...
	check_near("F.SilkS text starts at the anchor x", fb.position.x, 10.0)
	check_near("F.SilkS text ends at anchor + width", fb.end.x, 21.5)
	check_near("B.SilkS text starts at anchor - width", bb.position.x, -1.5)
	check_near("B.SilkS text ends at the anchor x", bb.end.x, 10.0)
...
func _run_verbs() -> void:
	...
	if bool(reply.get("success", false)):
		...
		check("the reply carries the mirrored bounds",
			absf(float(bounds.get("min_x_mm", 0.0)) - (-1.5)) < EPS
				and absf(float(bounds.get("max_x_mm", 0.0)) - 10.0) < EPS,
			str(bounds))
